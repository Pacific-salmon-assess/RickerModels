...
survparb<-data.frame(parameters=c("a[2010]","a[2011]","a[2012]","a[2013]","b","$\\sigma$"),
  MLE=c(exp(obj_surv$report()$alpha*SRsurv$Age2Surv[meanct]),obj_surv$report()$beta,obj_surv$report()$SigObs),
  median=c(apply(aall,1,median)[meanct],median(beta_surv),median(sig_surv)),
  low=c(apply(aall,1, function(x){quantile(x, probs=0.025)})[meanct], quantile(beta_surv, probs=0.025), quantile(sig_surv, probs=0.025)),
  high=c(apply(aall,1,function(x){quantile(x, probs=0.975)})[meanct], quantile(beta_surv, probs=0.975), quantile(sig_surv, probs=0.975)))
#
...
knitr::kable(survparb, caption = "Parameter estimates for model with survival as a covariate.")
...
survfecB<-list(
  ...
  nchain=3,
  iter=iteracs,
  lowbd=c(0.1,-10,-10,-13.5,-6.0),
  hibd=c(50.0,10,10,-8.5,5.0)
)
...
unique(survfecdf$parameters)

a_survfec <- (survfecdf$value[survfecdf$parameters=="alpha"])
slfec <- survfecdf$value[survfecdf$parameters=="sl_surv"]
slsurv <- survfecdf$value[survfecdf$parameters=="sl_fec"]

aallfec <- matrix(NA, nrow=length(SRsurv$Age2Surv), ncol=length(a_survfec))

for(i in seq_along(SRsurv$Age2Surv)){
  aallfec[i,] <- a_survfec+slfec*feczscore[i]+slsurv*survzscore[i]
}
...
survfecparb<-data.frame(parameters=c("a[2010]","a[2011]","a[2012]","a[2013]","b","$\\sigma$"),
  MLE=c(exp(obj_surv_fec$report()$alphat[meanct]),obj_surv_fec$report()$beta,obj_surv_fec$report()$SigObs),
  median=c(apply(aallfec,1,median)[meanct],median(beta_survfec),median(sig_survfec)),
  low=c(apply(aallfec,1, function(x){quantile(x, probs=0.025)})[meanct], quantile(beta_survfec, probs=0.025), quantile(sig_survfec, probs=0.025)),
  high=c(apply(aallfec,1,function(x){quantile(x, probs=0.975)})[meanct], quantile(beta_survfec, probs=0.975), quantile(sig_survfec, probs=0.975)))
