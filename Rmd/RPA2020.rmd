...
TODO =). probably need to centralize the variables.
...
aics<-data.frame(model=c("simple ricker", "autocorrelation in Recruitment", "time-varying productivity",
  "model with survival to age 2", "model with survival and fecundity"),
 AIC= c(TMBAIC(opt), TMBAIC(optar),  TMBAIC(opt_timevar), TMBAIC(opt_surv), TMBAIC(opt_surv_fec)),
 deltaAIC= c(TMBAIC(opt), TMBAIC(optar), TMBAIC(opt_timevar), TMBAIC(opt_surv), TMBAIC(opt_surv_fec) )-min(c(TMBAIC(opt), TMBAIC(optar), TMBAIC(opt_timevar),TMBAIC(opt_surv),TMBAIC(opt_surv_fec) )))
knitr::kable(aics)
