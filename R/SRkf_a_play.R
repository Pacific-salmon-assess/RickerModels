...
obj3<-MakeADFun(mydata3,parameters_recursive,random="alpha", DLL="Rickerkf_ratiovar")
...
parameters_recursive_loga <- list(
  logalphao=log(a_srm),
  logbeta = log(b_srm),
  rho=.2,
  logvarphi= 0.1,
  logalpha=log(rep(0.9,length(SR$R)))
  )
...
obj3_aprior<-MakeADFun(mydata3,parameters_recursive_loga
  ,random="logalpha",DLL="Rickerkf_ratiovar_aprior")
#newtonOption(obj3_aprior, smartsearch=FALSE)

obj3_aprior$fn()
obj3_aprior$gr()

opt_apr<-nlminb(obj3_aprior$par,obj3_aprior$fn,obj3_aprior$gr)
repkf3_aprior<-obj3_aprior$report()


sdreport(obj3_aprior)
...
dfac<-data.frame(broodyear=rep(SR$BroodYear,8),
  a=c(posterior_recursive3$fit_summary$summary[5:34,6],
    exp(posterior_recursive3_aprior$fit_summary$summary[5:34,6])
  ...
  lower=c(posterior_recursive3$fit_summary$summary[5:34,4],
    exp(posterior_recursive3_aprior$fit_summary$summary[5:34,4])),
  upper=c(posterior_recursive3$fit_summary$summary[5:34,8],
    exp(posterior_recursive3_aprior$fit_summary$summary[5:34,8])))
